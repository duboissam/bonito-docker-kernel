...
DEVICE="${1:-}"
KERNEL_DIR="${2:-}"
if [[ -z "$DEVICE" || -z "$KERNEL_DIR" ]]; then
  echo "Usage: $0 <device> /path/to/kernel" >&2
  exit 2
fi
...
[[ -f "$DEFCONFIG_PATH" ]] || { echo "$DEFCONFIG_PATH missing"; exit 1; }

mkdir -p "$OUT"
cp "$DEFCONFIG_PATH" "$DEFCONFIG_PATH.backup.$(date +%Y%m%d-%H%M%S)"
...
enable() {
  if symbol_exists "$1"; then
    scripts/config --file "$DEFCONFIG_PATH" --enable "$1"
...
echo "Device: $DEVICE_ID ($DEVICE_LABEL)"
